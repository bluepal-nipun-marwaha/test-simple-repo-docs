...
- **Input**: None
- **Output**: List of formatted calculation strings
- **Format**: `"number1 operation number2 = result"` or `"n! = result"`
...
- **Trigger**: When attempting to calculate the factorial of a negative number
- **Handling**: Graceful error message display
- **Recovery**: User can try again with a non-negative integer


### Invalid Input for Factorial
- **Error**: `ValueError` for non-integer input
- **Trigger**: When entering non-integer values for factorial calculation
- **Handling**: Error message display
- **Recovery**: User prompted to enter a valid integer
